#!/bin/bash
# Documentation Audit Script for doc4microservices
# Usage: ./audit_links.sh [--quick|--full|--links|--structure]
...
check_link_in_file() {
    ...
    while IFS=: read -r filepath line link; do
        ...
        local path
        path=$(echo "$link" | sed 's/.*(\([^)]*\.md[^)]*\)).*/\1/' | sed 's/#.*//')
...
check_links() {
    ...
    # Check links in all markdown files including README.md and CLAUDE.md in root
    echo "Checking root documentation files..."
    for file in README.md CLAUDE.md; do
        if [ -f "$file" ]; then
            check_link_in_file "$file"
        fi
    done
...
check_files() {
    ...
    # Find all referenced files in markdown (both root and docs)
    (grep -rh '`[^`]*\.md`' "$DOCS_DIR" 2>/dev/null || true;
     grep -h '`[^`]*\.md`' README.md CLAUDE.md 2>/dev/null || true) | \
...
check_structure() {
    ...
    if [ -f "$DOCS_DIR/INDEX.md" ]; then
        diff <(grep -o '[^(]*\.md' "$DOCS_DIR/INDEX.md" | sort | uniq) \
             <(find "$DOCS_DIR" -name "*.md" -printf "%P\n" | sort) || true
    fi
}
...
check_duplicates() {
    ...
    echo "Duplicate top-level headings:"
    grep -rh '^# ' "$DOCS_DIR" 2>/dev/null | sort | uniq -d
}
